...
import { routeYears, type RouteYears } from "@/lib/scenarios/vintage";
...
/**
 * How the route shares behind every scenario in this (year, commodity) are
 * dated — the second half of what an exposure row rests on, beside the BACI
 * trade year.
 *
 ...
 * the same promises and reads the `data_year`/`source_year` columns they
 * already carry. The derivation is the scenario panel's own `routeYears`, so
 * the two surfaces cannot date the same shares differently.
 */
export const loadRouteYears = lruLoader(
  8,
  async (year: number, commodity: Commodity): Promise<RouteYears> => {
    const defs = SCENARIOS.filter(
      (s) => s.commodities.includes(commodity) && isScenarioActive(s, year),
    );
