...
| S6 | merged 2026-09-19 — **shares await maintainer approval before `main`** | 7 scenarios from the verifiers' ship lists only: malacca (+lng), suez (+lng), bab_el_mandeb (+lng), turkish_straits, keystone, enbridge_mainline, espo_spur. disruption_route 72 → 521 rows, duplicate-key guard. BACI reconciliation: Malacca Gulf→East Asia 9.69 mb/d 2024 vs EIA 9.96; Suez 1.93; Bab 0.63. `routeKeyFor` generalised to `<id>_lng`. No shipped share changed |
| S7 | merged 2026-09-19 | `embed`/`controls` are view flags outside AppState, preserved across the store's URL rewrites; `frame-ancestors *` on `/`, `'none'` on every other page. Fix pass found two app bugs (open-full-map href stuck at `/`; double-collapsed layer list) |
| S2 | merged 2026-09-19 | BACI arcs, top 150 pairs (82–87% of crude, 94–100% of LNG volume, every year 1995–2024); with `focus`, all of that country's flows above 0.1%. Flows preset: trade_flows on, lng_voyages off |
...
| T2 | held | see R3 + RV |
| T1 | merged 2026-09-19 | `sev=`, `scenario2=`, `view=exporters` (each omitted at its default; old URLs byte-identical); combined results shown as a range; all 8 S5-review findings closed. Recovered from a WIP snapshot after a session restart killed the first agent |
| T3 | merged 2026-09-19 | Context block: EU storage scale comparison + Comtrade recency check; view-only data cannot reach the CSV |
| Final review | done 2026-09-19 | 2 critical (LNG factor was LHV labelled HHV → 15.644 TWh/Mt; a 2020 UK storage reading shown as latest), 6 high — all fixed in two batches. **Final: 139/139 e2e, 923 Vitest, 306 pytest.** Resume prompt: `docs/superpowers/HANDOFF.md` |
