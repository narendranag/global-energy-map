...
- **Next — data freshness (free sources)** — planned 2026-09-17, not started: `docs/superpowers/plans/2026-09-17-data-freshness-upgrade.md`. **P0: the GEM GOIT/GGIT download URLs 404** since GEM reorganised its public bucket (~2026-07-02), so `build_all --ingest` is broken — the deployed site is unaffected (`public/data/` is committed). Then: refresh stale pins (GOGET is still July 2023, EI 2026 edition is out), add GIE AGSI/ALSI daily EU gas+LNG, UN Comtrade monthly, EIA. Paid data is ruled out for now on redistribution-licence grounds, not cost — see `docs/commercial-data-options.md`.
