...
Idempotent: append_kind drops prior kind='port' rows before appending.
Field mapping lives in scripts.transform._netl_points (shared with storage).
Port capacity has no consistent unit in NETL, so capacity_unit is NULL.
...
def build(gdf: gpd.GeoDataFrame) -> pd.DataFrame:
    return build_netl_points(gdf, kind=KIND, id_prefix="netl-port-", capacity_unit=None)
...
def main() -> None:
    ...
    print(f"wrote {ASSETS_PATH}  new_port_rows={len(out)}")
