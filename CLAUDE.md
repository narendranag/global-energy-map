...
- **Data freshness (free sources)** — _in progress, 2026-09-17_: `docs/superpowers/plans/2026-09-17-data-freshness-upgrade.md`. Done: Track 0 (GEM ingests unbroken, liveness test, raw snapshots archived to R2), Track A (GOGET March 2026, EI 2026, OSM/NETL/BACI checked), B.1 GIE AGSI/ALSI (live gas-storage layer), B.2 UN Comtrade monthly (data only, no UI), C.1 Data vintage panel section. Remaining: Comtrade presentation, B.3 EIA, C.2 staleness markers, C.3 `/methodology` recency table. Paid data is ruled out for now on redistribution-licence grounds, not cost — see `docs/commercial-data-options.md`.
