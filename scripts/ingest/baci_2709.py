"""Ingest BACI bilateral trade data (HS92) filtered to crude petroleum (HS6=270900).
...
filters to HS6 product code 270900 (crude petroleum), and saves one small filtered
...
Usage:
    uv run python -m scripts.ingest.baci_2709 [--force]
"""
...
def _download_and_filter_year(
    ...
    print(f"  {year}: filtering to HS6=270900 in chunks...", flush=True)
...
def download_all(force: bool = False) -> list[Path]:
    """Download country_codes + all BACI year CSVs filtered to HS6=270900.
    ...
    for year, comp_size, local_offset in _YEAR_ENTRIES:
        dest = RAW_DIR / f"BACI_HS92_Y{year}_crude.csv"
...
def main() -> None:
    ...
    print(f"Ingesting BACI HS92 crude oil (HS6=270900) into {RAW_DIR}/")
