# Global Energy Map — Phase history

> Moved verbatim from `docs/methodology.md` on 2026-09-10 (Phase 9). This is the phase-by-phase record of what shipped and the caveats that applied **at that time**; several claims below were superseded by later phases. For the current-state methodology see `docs/methodology.md` (rendered at `/methodology`); for the source inventory see `docs/data-sources.md`.
...
- **Drop the year slider; scenario-first framing, 2026-09-21** (`97c0688`…`9aa8c3c`, plan [`docs/superpowers/plans/2026-09-21-drop-year-slider.md`](superpowers/plans/2026-09-21-drop-year-slider.md), recommendation [`docs/superpowers/briefs/2026-09-20-scenario-first-framing.md`](superpowers/briefs/2026-09-20-scenario-first-framing.md)) — the year slider is gone from the UI; every layer defaults to its own latest data and a pinned `?year=` link shows an "as of" chip instead. The scenario panel states a visible headline and an open-by-default "Data behind this result" vintage disclosure; README, site metadata, the header tagline, `llms.txt` and `CITATION.cff` lead with the scenario question rather than the asset inventory. Every information surface (layer badges, country panel, tooltips, CSV headers) states the vintage of the data behind it. `disruption_route` gained a nullable `data_year` distinct from `source_year` (73 of 522 rows dated). Every year constant (`TRADE_FIRST_YEAR`, `TRADE_LAST_YEAR`, reserves/LNG-T3 spans) is now derived from catalog `coverage` rather than hand-typed, guarded by `tests/python/test_data_integrity.py`. Version 1.1.0 throughout. The e2e suite followed the same day: `time.spec.ts`/`year-relevance.spec.ts` removed, `tests/e2e/as-of.spec.ts` added; full suite on a production build was 145 passed for the vintage work, and the final label-only change (dropping bracketed years from the intro questions) shipped without a fresh e2e run, at the maintainer's direction.
