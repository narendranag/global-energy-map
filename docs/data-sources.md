...
- **HS 271111** (liquefied natural gas) for the LNG axis of Hormuz, Malacca, Suez and Bab el-Mandeb (each stored under its own `${scenario}_lng` disruption id)
...
- **As-of:** 2026-09-19 (consolidated EIA + IEA + Argus + CER + USCC references; R1/R2/RV research and verification)
- **Where it lands:** `disruption_route.parquet` (each row carries `source_title`, `source_url`, `source_year`, `data_year`)
- **Layers/scenarios using it:** all 15 scenarios — Hormuz crude, Hormuz LNG (`hormuz_lng`), Druzhba, BTC, CPC, Malacca (crude + `malacca_lng`), Suez+SUMED (crude + `suez_lng`), Bab el-Mandeb (crude + `bab_el_mandeb_lng`), Turkish Straits, Keystone, Enbridge Mainline, ESPO Skovorodino-Mohe spur (`espo_spur`)

**What we ingest:** static routing-share tables that encode how each exporter's crude or LNG flows through each chokepoint or pipeline. Example: Saudi Arabia's crude is 88% Hormuz-dependent, 12% bypasses via East-West pipeline. Druzhba routing shares: DEU and POL 47% of Russian crude (IEA northern-branch volume allocated pro-rata), BLR/SVK/HUN/CZE 100%. Each row in `disruption_route.parquet` carries its citation and derivation (`source_title`, `source_url`, `source_year`, `data_year`, `source_note`); six shares were revised to source-derived values on 2026-09-10. Since 2026-09-21 the two years are kept apart: `source_year` is when the document was published, `data_year` the year of the flows it describes (73 rows have one; structural, capacity-derived and unsourced rows are NULL by construction). The site states the data year and keeps the publication year as the secondary date. Since 2026-09-11 the file also carries 54 structural share-0 pairs (42 crude, 12 LNG) — one for each Hormuz exporter × other Gulf-coast country — so intra-Gulf cargoes, which never cross the strait, are not counted as exposed (`GULF_COASTAL` in `scripts/transform/build_disruption_routing.py`).

**2026-09-19 (S6, post-launch UX/scenarios plan):** added five new chokepoints/pipelines after independent verification of the research notes (`docs/superpowers/research/chokepoints.md`, `oil-pipelines.md` — the verifier's corrected values are what shipped, not the first-pass researcher's). Unlike Hormuz, Malacca/Suez/Bab el-Mandeb/Turkish Straits are **direction-dependent**: a share applies only to the (exporter, importer-region) pairs whose cargo actually needs that route (e.g. Gulf crude to Europe crosses Bab el-Mandeb, to East Asia it does not), so most new rows are region-expanded exporter→importer pairs rather than exporter-wide wildcards. Shipped: Malacca (Gulf-7 → East Asia minus Indonesia = 1.00, → South Asia = 0.00, USA → East Asia = 0.60; Qatari LNG → East Asia = 1.00, no Australia row — its LNG does not transit this strait); Suez+SUMED (Gulf-6 → Europe/Mediterranean + USA = 1.00 structural, not the EIA "85%" composition figure the first-pass note misapplied; Qatari LNG → Europe = 1.00); Bab el-Mandeb (Gulf-5 → Europe/Mediterranean = 1.00, explicit Saudi Arabia = 0.00 since its Europe-bound crude loads at Yanbu north of the strait; Qatari LNG → Europe = 1.00); Turkish Straits (Kazakhstan 0.80, consistent with the shipped CPC row, plus explicit share-0 pairs for Bulgaria/Romania — Black Sea importers that never reach the Bosporus — and China, reached by pipeline); Keystone (Canada→USA 0.14, Canada Energy Regulator); Enbridge Mainline (Canada→USA 0.60, CER's "58% of all Canadian crude exports" converted to a CAN→USA basis); ESPO's Skovorodino-Mohe spur only (Russia→China 0.28, design capacity, explicitly excluding the Kozmino seaborne leg, which BACI cannot separate from other Russian crude reaching China by tanker since 2022). `disruption_route.parquet` now has 525 rows.

**Held, not shipped** (researched but too weakly sourced, or need a schema change first — see the research notes' "ship/hold" sections): Danish Straits (the only defensible number, a Russia-wide wildcard, would assert more crude crossing the strait than EIA measures transiting it in total); any Russia-wide wildcard on Suez or Turkish Straits (BACI has no port-of-loading field, so Russia's Baltic/Black Sea/Pacific export split cannot be separated, and it is itself changing month to month in 2026); Panama Canal (its dominant commodity, LPG, is outside this project's HS-code schema, and the crude/LNG shares available contradict the Malacca destination breakdown); Saudi East-West/Petroline (a flat exporter share would apply uniformly to every Saudi buyer, which is wrong in the one direction that matters — Yanbu-loaded crude serves Europe, not Asia); Kirkuk-Ceyhan (needs a year-bounded share — the pipeline was shut 2023-03 to 2025-09, entirely inside the app's 1990–2024 year range for 2023–2024 — and would require updating the shipped Hormuz IRQ 0.90 share in the same commit, which is the maintainer's call, not made here).

**Coverage gaps:**
- **Routing shares are fixed across all years** (except the not-yet-used `activeYears` schema hook added for a future year-bounded route) — they don't track maintenance windows, sanctions regimes, or renegotiations of joint-venture operating agreements.
...
- **Layers/scenarios using it:** Gas storage (EU) — the latest gas day, independent of the selected year
