...
- **GEM multi-unit pids, not verbatim duplicates.** An early comment mischaracterized GGIT's per-pid repeats as byte-identical duplicate features; they are actually one feature per liquefaction/regasification train sharing a terminal-level `pid`, and the terminal-level capacity field is already the terminal total (48 of 84 duplicated pids differ in status/start-year/owner/tracker-custom/geometry across their per-unit rows). `_load_gem` collapses each multi-unit pid to one record (operating preferred over in-construction, then highest capacity — the same deterministic rule LNG-T3's name collapse uses) before the 25 km proximity dedup against LNG-T3, leaving a GEM supplement of **8** terminals (the plan estimated ~10).
- **Orphan-row purge.** A rebuild also dropped 131 all-null orphan rows (source/source_version populated, everything else null) that an earlier index-alignment bug had baked into `assets.parquet`; `main()` now drops null-`kind` rows unconditionally and asserts uniqueness/non-null `kind` on the full table.
- **Final counts** (vs. the plan's Task 5 estimates of ~330 LNG-T3 + ~10 GEM ≈ ~340 total): **305 LNG-T3 + 8 GEM = 313 LNG terminals** (74 lng_export / 239 lng_import), out of `assets.parquet`'s **37,477** total rows (not the pre-purge 37,608). LNG-row coverage: capacity 311/313, `commissioned_year` 306/313 (**97.8%**, plan estimated ~88%). Voyage/trade/terminal-daily parquet counts matched the plan's expectations exactly: 17,592 / 16,691 / 16,115 rows; 159 of 471 unique terminal names in `LNG_terminal.csv` have measured throughput. **(Superseded by the post-ship fix below.)**
- **Post-ship review fix (2026-09-09): name-equality dedup pre-pass + voyage_id disambiguation.** The 25 km haversine dedup above was proximity-only, so a same-name GEM/LNG-T3 pair with coordinates further apart than 25 km survived as two rows (e.g. "Ichthys FLNG Terminal": GEM and LNG-T3 records ~440 km apart, same country). Added a name-equality pre-pass (`name_matches_in_country()`) ahead of the haversine pass, plus an assert that `(country_iso3, name)` is unique across the combined LNG frame — this drops one additional GEM row, bringing the GEM supplement from 8 to **7** and the terminal total from 313 to **312** (73 lng_export / 239 lng_import; capacity 310/312; `commissioned_year` 305/312, still 97.8%; `assets.parquet` now **37,476** rows). Separately, `voyage_id` (start_date + IMO + voyage_type) was found to collide 13 times (30 rows) in the shipped voyage parquet; `make_unique_ids()` now appends a per-collision-group suffix, with an `assert out["voyage_id"].is_unique` before writing. Voyage/trade/terminal-daily row counts are unchanged (17,592 / 16,691 / 16,115).
