...
| `disruption_route.parquet` | 522 | project-derived, per-row citations | `disruption_id, kind, exporter_iso3, importer_iso3 (null = all), share, source, source_title, source_url, source_year, data_year (nullable: the year of the flows the document describes; NULL on structural and unsourced rows), source_note`; 18 cited shares plus 54 intra-Gulf pairs pinned to share 0 (Hormuz); `hormuz_lng` rows are used on the gas axis |
...
SELECT exporter_iso3, importer_iso3, share, source_title, source_year, data_year
