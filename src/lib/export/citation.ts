...
import { formatSpan } from "@/lib/data/vintage";
...
/**
 * The bundled route-share citations behind a live scenario selection —
 * every row of both scenarios, under each one's own route key (a gas run
 * reads `hormuz_lng`, the same lookup `loadScenarioInputs` does).
 *
 * This is the *static* copy of the same rows the engine loads, generated by
 * `build_catalog.py`, so a caller that only needs the shares' publication
 * years (the embed chip, the citation summary) can state them synchronously
 * instead of waiting on a parquet read. The engine's own result stays the
 * source of every number.
 */
export function bundledSharesFor(
  ...
  return ids.flatMap((id) => {
    const key = routeKeyFor(id, commodity);
    return key === null ? [] : sharesFor(key, rows);
  });
}
 ...
 * A series also says what it covers ("…, data 1995–2024, as of 2026-01-01."):
 * `as_of` is the release date, which says nothing about where the data ends.
 */
export function sourceCitationLine(e: CatalogEntry): string {
  const covers = (e.coverage ?? []).map(formatSpan).join("; ");
  const data = covers === "" ? "" : `, data ${covers}`;
  return `${e.source_name} — ${e.label}${data}, as of ${e.as_of}. Licence: ${e.license}. ${e.source_url}`;
}
